...
export async function buildBlenderLeasePrefill(
  ...
  if (lease.state === "ready" && lease.gpu_name) {
    ...
    lines.push(`7. Verify early: get_viewport_screenshot after the first 2-3 parts (auto-frames all visible meshes). Use get_viewport_screenshot(from_camera=True) to check camera framing. PREFER a preview render (rule 2) for definitive framing — viewport screenshots approximate the camera view but don't show render materials/lighting. Set up lighting + camera BEFORE detailed geometry.`);
