...
export async function buildBlenderLeasePrefill(
  ...
  if (lease.state === "ready" && lease.gpu_name) {
    ...
    lines.push(
      `Workflow after every meaningful change: (1) save via execute_code bpy.ops.wm.save_as_mainfile(filepath="/root/blender/scene.blend"), (2) trigger a quick EEVEE preview render (16 samples, 960x540) by running this in your bash tool: cd /app/blender && uv run --project /app/blender python /app/blender/run.py '${instanceFolder}' --request request.json — but FIRST write a request.json into the instance folder: echo '{"op":"preview","settings":{"samples":16,"resolution_x":960,"resolution_y":540}}' > '${instanceFolder}/request.json'. The run.py script uses the direct socket (600s budget, not the ~120s MCP bridge cap) and writes state.json phases the canvas polls. (3) poll state.json (phase goes rendering → gpu_ready) and the renders[] preview entry at exports/preview.png. The host syncs the preview from the GPU to the workspace within ~5s. For QUICK previews you can also use execute_blender_code with bpy.ops.render.render(write_still=True) — but run.py is preferred for complex scenes.`,
    );
    lines.push(
