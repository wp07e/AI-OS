...
class BlenderMCPServer:
    ...
    # ── Scene-diff interception ─────────────────────────────────────────────
    #
    # Problem being solved: the agent repeatedly skips "inspect after every
    # operation," building 10+ parts before noticing positions/scales/parenting
    # are wrong (e.g. transform_apply zeroing locations, unparented legs).
    # Documentation is advisory and gets ignored under load.
    #
    # Solution: for every MUTATING command, snapshot a per-object manifest
    # before and after the handler runs, diff it, and append the diff to the
    # tool result the agent receives. The agent is then forced to see changes
    # (location deltas, new unparented parts, zero-vertex meshes, baked
    # transforms) in its context window after every call — Step 2 ("inspect
    # after every step") becomes structural rather than advisory.
    #
    # Read-only queries (the get_* commands) skip the diff entirely.

    # Commands that only READ state — no manifest needed (keeps output clean
    # and avoids the cost of snapshotting on every get_viewport_screenshot).
    ...
    def _scene_manifest(self):
        ...
        try:
            for obj in bpy.data.objects:
                if obj.type == 'MESH':
                    manifest[obj.name] = {
                        "loc": self._round3(obj.location.x),
                        "loc_y": self._round3(obj.location.y),
                        "loc_z": self._round3(obj.location.z),
    ...
    def _format_scene_diff(self, before):
        """Diff current state against `before`; return a human-readable block.

        Empty string when nothing relevant changed (so read-only-ish mutations
        don't spam the result). Flags the exact failure modes that have burned
        sessions: location resets (transform_apply footgun), unparented new
        parts (detached legs), and zero-vertex meshes (corrupted/invisible).
        """
        after = self._scene_manifest()
        lines = []

        # New objects since the snapshot — flag unparented meshes loudly.
        for name in after:
            if name not in before:
                info = after[name]
                if "verts" in info:  # a mesh
                    tag = "parent=None" if info.get("parent") is None else f"parent={info.get('parent')}"
        ...
        # Changed objects — detect the transform_apply location-zero footgun.
        for name in before:
            if name not in after:
                continue
            ...
            changes = []
            # Location reset to origin after a scale-apply is the classic
            # transform_apply(scale=True) footgun: it bakes scale into verts
            # and zeroes location. Surface it explicitly so the agent fixes
            # positions immediately rather than after 10 more parts.
            if (b["loc"], b["loc_y"], b["loc_z"]) != (a["loc"], a["loc_y"], a["loc_z"]):
                if (a["loc"], a["loc_y"], a["loc_z"]) == (0.0, 0.0, 0.0) and (b["loc"], b["loc_y"], b["loc_z"]) != (0.0, 0.0, 0.0):
                    changes.append(f"location → (0,0,0) ⚠ RESET (likely transform_apply — re-position it)")
                else:
                    changes.append(f"location ({b['loc']},{b['loc_y']},{b['loc_z']})→({a['loc']},{a['loc_y']},{a['loc_z']})")
            if b["scl"] != a["scl"]:
                changes.append(f"scale_x {b['scl']}→{a['scl']}")
            if b["parent"] != a["parent"]:
                changes.append(f"parent {b['parent']}→{a['parent']}")
            if b["verts"] != a["verts"]:
                changes.append(f"verts {b['verts']}→{a['verts']}")
            if changes:
    ...
    def _execute_command_internal(self, command):
        ...
        if handler:
            # Snapshot before mutating commands so we can diff afterward. This
            # is what makes "inspect after every step" unavoidable: the diff
            # is appended to the tool result the agent reads, so location
            # resets, unparented parts, and zero-vertex meshes surface in the
            # agent's context window immediately — not 10 parts later.
            mutating = cmd_type not in self._READ_ONLY_COMMANDS
            before = self._scene_manifest() if mutating else None
            try:
